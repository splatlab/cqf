...
static inline uint64_t run_end(const QF *qf, uint64_t hash_bucket_index)
{
	...
	const uint64_t runend_index = SLOTS_PER_BLOCK * runend_block_index + runend_block_offset;
...
void qf_dump_metadata(const QF *qf) {
    printf("Slots: %" PRIu64 " Occupied: %" PRIu64 " Elements: %" PRIu64 " Distinct: %" PRIu64 "\n",
                 qf->metadata->nslots,
                 qf->metadata->noccupied_slots,
                 qf->metadata->nelts,
                 qf->metadata->ndistinct_elts);
    printf("Key_bits: %" PRIu64 " Va" PRIu64 "e_bits: %" PRIu64 " Remainder_bits: %" PRIu64 " Bits_per_slot: %" PRIu64 "\n",
...
void qf_dump(const QF *qf)
{
    ...
    for (i = 0; i < qf->metadata->nblocks; qf_dump_block(qf, i++));
                                                                            ...
                                                                            *remainder, uint64_t *count)
{
    ...
    *remainder = rem = get_slot(qf, index);
    ...
    cnt = 0;
    base = (1ULL << qf->metadata->bits_per_slot) - ((rem != 0) + 1);
...
bool qf_resize_malloc(QF *qf, uint64_t nslots)
{
    ...
    do {
        ...
        if (!qf_insert(&new_qf, key, value, count)) {
            fprintf(stderr, "Failed to insert key: %" PRIu64 " into the new CQF.\n", key);
            abort();
        }
    } while(!qfi_end(&qfi));
...
{
    return qfi->current >= qfi->qf->metadata->xnslots /*&& is_runend(qfi->qf, qfi->current)*/;
}
...
void qf_multi_merge(const QF *qf_arr[], int nqf, QF *qfr)
{
    ...
    if (!qfi_end(&qfi_arr[0])) {
        ...
        DEBUG_CQF("Num of iterations: %" PRIu64 "\n", iters);
    }
