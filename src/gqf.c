...
insert_replace_slots_and_shift_remainders_and_runends_and_offsets(
  ...
  if (ninserts > 0) {
    ...
    shift_runends_tombstones(qf, insert_index, empties[ninserts - 1] - 1,
                             ninserts);
...
bool qf_malloc_advance(QF *qf, uint64_t nslots, uint64_t key_bits,
  ...
  uint64_t init_size =
      qf_init_advanced(qf, nslots, key_bits, value_bits, hash, seed,
                       tombstone_space, nrebuilds, buffer, total_num_bytes);
