...
#define QF_METADATA_WORDS_PER_BLOCK ((QF_SLOTS_PER_BLOCK + 63) / 64)
	...
	typedef struct quotient_filter_metadata {
		...
		uint64_t nrebuilds;      		// Number of rebuild per loop.
		uint64_t rebuild_slots;  		// Number of slots to be rebuilt.
		uint64_t key_bits;
		uint64_t value_bits;
		...
		uint64_t n_new_tombstones;	// Number of the tombstones generated by a deletion.
