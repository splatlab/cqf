...
#define QF_DOESNT_EXIST (-3)
	...
	****************************************/
	...
	uint128_t qf_get_hash_range(const QF *qf);
...
#define QF_INVALID (-4)
	...
	********************************************/
	...
	typedef struct quotient_filter_merge_iterator quotient_filter_merge_iterator;
	typedef quotient_filter_merge_iterator QFmi;

	/* Initialize an iterator starting at the given position. */
	bool qf_merge_iterator(const QF *qf, QFmi *qfmi, uint64_t niterators, QFi qfi[]);

	/* Return the iterator for the next element. */
	int qfmi_get_current(QFmi *qfmi, QFi *qfi);
	
	/* Advance to next entry.  Returns whether or not another entry is
		 found.  */
	int qfmi_next(QFmi *qfmi);
